...
3. [硬件资源与引脚](PINOUT.md)：理解 MCU 外设和引脚占用。
4. [驱动接口规范](DRIVER_API.md)：理解现有驱动和新驱动的接口要求。
5. [主从通信协议](PROTOCOL.md)：理解双 MCU 数据交换。
6. [主控安全状态](SAFETY.md)：理解电机放行和失联行为。
7. [构建与验证](BUILD.md)：建立本地编译环境。
8. [自动测试说明](TESTING.md)：理解质量门槛和测试范围。
9. [开发维护指南](DEVELOPMENT_GUIDE.md)：按标准流程修改项目。
10. [运行维护手册](MAINTENANCE.md)：定位常见构建、通信和硬件问题。
11. [发布流程](RELEASE.md)：生成、验证和烧录匹配固件。
12. [后续路线](ROADMAP.md)：了解当前边界和下一阶段工作。
