...
11. [构建与验证](BUILD.md)：建立本地编译环境。
12. [自动测试说明](TESTING.md)：理解质量门槛和测试范围。
13. [开发维护指南](DEVELOPMENT_GUIDE.md)：按标准流程修改项目。
14. [运行维护手册](MAINTENANCE.md)：定位常见构建、通信和硬件问题。
15. [发布流程](RELEASE.md)：生成、验证和烧录匹配固件。
16. [后续路线](ROADMAP.md)：了解当前边界和下一阶段工作。
